# Primeiros Passos em TI - UNIVESP

Este repositório foi criado para registrar a minha evolução prática no Bacharelado em Tecnologia da Informação pela UNIVESP e minhas experiências com ferramentas de tecnologia aplicadas ao mercado real.

## 🛠️ Aplicação Prática Atual (Case Real)
Recentemente, utilizei Engenharia de Prompts com IA (Gemini) para otimizar e estruturar uma proposta técnica de **projeto luminotécnico** para um cliente real da minha empresa, a **Resolve Soluções**.

### Resultados:
* Agilidade na elaboração da proposta comercial.
* Linguagem técnica clara, profissional e direcionada às necessidades do cliente.
* Demonstração de como a tecnologia pode acelerar o ecossistema de prestação de serviços (Elétrica e Refrigeração).

## 🚀 Próximos Objetivos:
* Avançar nas 4 semanas do programa de acolhimento AIA (UNIVESP).
* Iniciar os fundamentos de Lógica de Programação e Desenvolvimento Web (HTML/CSS/JavaScript).
* Construir automações simples de atendimento voltadas para pequenos negócios e prestadores de serviço.
* 
